...
real,allocatable:: ncan(:,:),p(:,:),c(:,:),rd(:,:),d(:,:),rr(:,:)
real,allocatable:: IDsq(:,:),ID(:,:),IR(:,:),IRR(:,:)
...
& IDsq(y-fy+1,m),ID(y-fy+1,m),IR(y-fy+1,m),IRR(y-fy+1,m)

1000 format(6x,i5,f8.2,5(f10.2,10x),4f10.2)

1 close(10)

open(20,file=fout)
write(20,'("year",13(6x,a4,7x,"sig"))')"Nc","Na","P","dsqP","D"," R","RR","IDsq","ID","IR","IRR"
